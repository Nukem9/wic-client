...
namespace MC_Algorithm
{
	...
	inline int BinarySearch(ITERATOR aFirst, ITERATOR aLast, const COMPARED& aCompared, const COMPARER& aComparer)
	{
		ITERATOR i = aFirst;
	...
	inline void Sort(ITERATOR aFirst, ITERATOR aLast, const COMPARER& aComparer)
	{
	...
	template <typename ITERATOR, typename COMPARER>
	inline void SortReverse(ITERATOR aFirst, ITERATOR aLast, const COMPARER& aComparer)
	{
		if (aFirst + 1 > aLast)	// Bail if one item or less
			return;
