...
public:
	...
	{
#if MC_HEAVY_DEBUG_MC_STRING_BOUNDSCHECK
		MC_ASSERTMSG(aIndex >= 0, "String boundscheck");
		MC_ASSERTMSG(aIndex == 0 || aIndex < GetBufferSize(), "String boundscheck");
#endif
	...
	{
#if MC_HEAVY_DEBUG_MC_STRING_BOUNDSCHECK
		MC_ASSERTMSG(aIndex == 0 || int(aIndex) < GetBufferSize(), "String boundscheck");
#endif
		return GetBuffer()[aIndex];
	}
...
protected:
	...
	template<class C, int S>	static void	InternalSetBuffer(MC_Str<C, S>& aStr, C* aPointer) { MC_ASSERTMSG(false, "not allowed on static strings!"); }
	...
	template<class C, int S>	static void	InternalSetBufferSize(MC_Str<C, S>& aStr, int aSize) { MC_ASSERTMSG(false, "not allowed on static strings!"); }
	...
	static void	InternalMakeUpper(TCHAR* aPointer, int aBufferSize) { if (aPointer && aBufferSize) MC_tcsupr_s(aPointer, aBufferSize); }
	static void	InternalMakeLower(TCHAR* aPointer, int aBufferSize) { if (aPointer && aBufferSize) MC_tcslwr_s(aPointer, aBufferSize); }
	static int	InternalCompare(const TCHAR* aString1, const TCHAR* aString2) { return MC_tcscmp(aString1, aString2); }
	static int	InternalCompareNoCase(const TCHAR* aString1, const TCHAR* aString2) { return MC_tcsicmp(aString1, aString2); }
	static bool	InternalEndsWith(const TCHAR* aString1, const TCHAR* aString2) { size_t len1 = MC_tcslen(aString1); size_t len2 = MC_tcslen(aString2); return len1 >= len2 ? (MC_tcscmp(aString1 + len1 - len2, aString2) == 0) : false; }
	static bool	InternalEndsWithNoCase(const TCHAR* aString1, const TCHAR* aString2) { size_t len1 = MC_tcslen(aString1); size_t len2 = MC_tcslen(aString2); return len1 >= len2 ? (MC_tcsicmp(aString1 + len1 - len2, aString2) == 0) : false; }
	static bool	InternalBeginsWith(const TCHAR* aString1, const TCHAR* aString2) { size_t len1 = MC_tcslen(aString1); size_t len2 = MC_tcslen(aString2); return len1 >= len2 ? (MC_tcsncmp(aString1, aString2, len2) == 0) : false; }
	static bool	InternalBeginsWithNoCase(const TCHAR* aString1, const TCHAR* aString2) { size_t len1 = MC_tcslen(aString1); size_t len2 = MC_tcslen(aString2); return len1 >= len2 ? (MC_tcsncicmp(aString1, aString2, len2) == 0) : false; }
	static int	InternalFind(const TCHAR* aString1, const TCHAR* aString2)
	{
		const TCHAR* ret = MC_tcsstr(aString1, aString2);
...
{
	if constexpr (S > 0)	// If static buffer
	{
		if (aSize >= S)
		{
			MC_ASSERTMSG(false, "The static string buffer is too small");
...
MC_Str<C, S>& MC_Str<C, S>::operator=(const TCHAR* aString)
{
	...
	if (newlen)
	{
		...
		{
			AllocData(newlen);
			newlen = std::min(newlen, GetBufferSize() - 1);
			C* dest = GetBuffer();
...
MC_Str<C, S>& MC_Str<C, S>::operator+=(const TCHAR* aString)
{
	...
	{
		ReallocData(newLen);
		otherLen = std::min(otherLen, GetBufferSize() - oldLen - 1);		//make sure we dont write to far on static buffer in release candidate with asserts disabled
		C* pBuf = GetBuffer();
