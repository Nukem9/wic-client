...
extern "C"
{
	...
	static_assert_size(word32, sizeof(u32));
	static_assert_size(word64, sizeof(u64));
...
MMG_CryptoHash
MMG_Tiger::GenerateHash(const void* theData, u32 theDataLength) const
{
	word64 res[3];
	tiger(static_cast<const word64 *>(theData), theDataLength, res);

	return MMG_CryptoHash(res, sizeof(res), GetIdentifier());
}

HashAlgorithmIdentifier MMG_Tiger::GetIdentifier() const
{
	return HASH_ALGORITHM_TIGER;
}

void
MMG_Tiger::Start()
{
	myState[0] = 0x0123456789ABCDEFLL;
	myState[1] = 0xFEDCBA9876543210LL;
	myState[2] = 0xF096A5B4C3B2E187LL;
	myDataLeftoversLength = 0;
	myTotalDataLength = 0;
}

void
MMG_Tiger::Continue(const char* theData, u32 theDataLength)
{
	myTotalDataLength += theDataLength;

	// if we have anything cached, pad it with theData and continue before doing anything else
	if (myDataLeftoversLength)
	{
		const u32 nBytes = std::min(64 - myDataLeftoversLength, theDataLength);

		memcpy(myDataLeftovers + myDataLeftoversLength, theData, nBytes);
		myDataLeftoversLength += nBytes;
		theData += nBytes;
		theDataLength -= nBytes;
	}

	// If we have a full cached block, hash it first
	if (myDataLeftoversLength == 64)
	{
		tiger_compress(reinterpret_cast<word64 *>(myDataLeftovers), myState);
		myDataLeftoversLength = 0;
	}
	...
	for (; i >= 64; i -= 64)
	{
		tiger_compress(reinterpret_cast<const word64 *>(theData), myState);
		theData += 64;
	}

	// If theData has any leftovers (i.e. the last <64 bytes left, then store them and use them later.
	if (i)
	{
		const u32 nBytes = std::min(i, 64 - myDataLeftoversLength);
...
MMG_Tiger::End()
{
	...
	for (j = 0; j < myDataLeftoversLength; j++)
		temp[j] = myDataLeftovers[j];
	...
	if (j > 56)
	{
		for (; j < 64; j++)
			temp[j] = 0;
		tiger_compress(reinterpret_cast<word64 *>(temp), myState);
		j = 0;
	}
	...
	((word64*)(&(temp[56])))[0] = ((word64)myTotalDataLength) << 3;
	tiger_compress(reinterpret_cast<word64 *>(temp), myState);
	return MMG_CryptoHash(myState, sizeof(myState), GetIdentifier());
}

#pragma region tiger.c
#pragma warning(push)
...
*
* Tiger has no usage restrictions nor patents. It can be used freely, with the reference
* implementation, with other implementations or with a modification to the reference
* implementation (as long as it still implements Tiger). We only ask you to let us know
* about your implementation and to cite the origin of Tiger and of the reference implementation.
*/
